...
class GlobalExceptionHandler {
    ...
    ErrorResponse handleValidation(MethodArgumentNotValidException ex, HttpServletRequest request) {
        log.debug("{} [{}]: {}", ex.getClass().getSimpleName(), request.getRequestURI(), ex.getMessage());
    ...
    ErrorResponse handleEmailExists(EmailAlreadyExistsException ex, HttpServletRequest request) {
        return ErrorResponse.of(409, ex.getMessage(), request.getRequestURI());
    }

    @ExceptionHandler(InvalidCredentialsException.class)
    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    ErrorResponse handleInvalidCredentials(InvalidCredentialsException ex, HttpServletRequest request) {
        return ErrorResponse.of(401, ex.getMessage(), request.getRequestURI());
    }

    @ExceptionHandler(InvalidTokenException.class)
    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    ErrorResponse handleInvalidToken(InvalidTokenException ex, HttpServletRequest request) {
        return ErrorResponse.of(401, ex.getMessage(), request.getRequestURI());
    }
